...
void nfkd_test(const NormalizationTest &test, const std::string &source, const std::string &test_nfc, const std::string &test_nfd,const std::string &test_nfkc, const std::string &test_nfkd){
	{
		...
		REQUIRE(test.nfkd == nfkd_from_nfkc);
...
void single_test(const NormalizationTest &test){
	std::string source = convert_utf32_to_utf8(test.source.data());
	std::string test_nfc = convert_utf32_to_utf8(test.nfc.data());
	std::string test_nfd = convert_utf32_to_utf8(test.nfd.data());
	std::string test_nfkc = convert_utf32_to_utf8(test.nfkc.data());
	std::string test_nfkd = convert_utf32_to_utf8(test.nfkd.data());
...
TEST_CASE("Normalization tests Part 0: Specific cases") {
	...
	auto testname_suffix = std::to_string(i) + ": " + test.test_name;
