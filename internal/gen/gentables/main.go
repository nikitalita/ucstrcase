...
func tablesFileName(dirname string) (string, string) {
	// gen.UnicodeVersion is set by the "-unicode" flag
	if name := buildTags[gen.UnicodeVersion()].filename; name != "" {
		return filepath.Join(dirname, name), filepath.Join(dirname, buildTags[gen.UnicodeVersion()].headername)
	}
...
func genCaseFolds(w *bytes.Buffer, hw *bytes.Buffer, firstValidHash bool) {
	folds := caseFolds
	...
	fmt.Fprint(hw, "\n")
	fmt.Fprintf(hw, "static const uint32_t _CaseFoldsSeed = 0x%04X;\n", seed)
	fmt.Fprintf(hw, "static const uint32_t _CaseFoldsShift = %d;\n", caseFoldShift)
	fmt.Fprint(hw, "\n")
...
func genFoldTable(w *bytes.Buffer, hw *bytes.Buffer, firstValidHash bool) {
	...
	fmt.Fprint(hw, "\n")
	fmt.Fprintf(hw, "static const uint32_t _FoldMapSeed = 0x%04X;\n", seed)
	fmt.Fprintf(hw, "static const uint32_t _FoldMapShift = %d;\n", foldMapShift)
	fmt.Fprint(hw, "\n")
	...
	for i := 0; i < foldMapSize-1; i++ {
		...
		for _, _ff := range folds {
			if uint32(i) == hash(uint32(_ff[0]), seed, foldMapShift) {
				ff = _ff
				break
			}
		}
...
// character.`
	...
	fmt.Fprintln(hw, "")
	fmt.Fprintf(hw, "static const uint32_t _FoldMapExcludingUpperLowerSeed = 0x%04X;\n", seed)
	fmt.Fprintf(hw, "static const uint32_t _FoldMapExcludingUpperLowerShift = %d;\n", foldMapShift)
	fmt.Fprintln(hw, "")
...
// is roughly ~4x faster.`
	...
	fmt.Fprint(hw, "\n")
	fmt.Fprintf(hw, "static const uint32_t _UpperLowerSeed = 0x%04X;\n", seed)
	fmt.Fprintf(hw, "static const uint32_t _UpperLowerShift = %d;\n", upperLowerTableShift)
	fmt.Fprint(hw, "\n")
	...
	for i := 0; i < upperLowerTableSize-1; i++ {
		...
		for _, _c := range cases {
			if uint32(i) == shiftHash(seed, uint32(_c.Rune), upperLowerTableShift) {
				c = _c
				break
			}
		}
...
static inline bool toUpperLowerSpecial(uint32_t r, uint32_t* upper, uint32_t* lower) {
	...
	for _, c := range special {
		...
		fmt.Fprintf(w, "\t\t*upper = %d;\n", c.Upper)
		fmt.Fprintf(w, "\t\t*lower = %d;\n", c.Lower)
	...
	fmt.Fprintf(w, "\t\t*upper = r;\n")
	fmt.Fprintf(w, "\t\t*lower = r;\n")
	fmt.Fprintln(w, "\t\treturn false;")
	fmt.Fprintln(w, "}")
}

func writeHeaderPrefix(w *bytes.Buffer) {
	const s = `
...
func writeTemp(name string, b []byte) {
	...
	if err != nil {
		// just make dir in current working directory
		dir = "tmp"
...
func listSourceFiles(dir string) []string {
	...
	for _, d := range des {
		...
		if d.Name() == "src" {
			// recurse into src
			a = append(a, listSourceFiles(filepath.Join(dir, d.Name()))...)
		} else if d.IsDir() && d.Name() == "include" {
...
func hashGenFiles(root, tablesFile, headerFile string) string {

	files := listSourceFiles(root)
	if len(files) == 0 {
		log.Fatal("no go source files in:", root)
	}
	// check if basename of the files match the basename of either tablesFile or headerFile, and if so, remove them
	for i := 0; i < len(files); i++ {
		if filepath.Base(files[i]) == filepath.Base(tablesFile) {
			files = append(files[:i], files[i+1:]...)
			break
		}
	}
	for i := 0; i < len(files); i++ {
		if filepath.Base(files[i]) == filepath.Base(headerFile) {
			files = append(files[:i], files[i+1:]...)
			break
		}
	}
...
func realMain() int {
	...
	// registered by the ucstrcase_gen/gen/gen package.
	...
	tablesFile, headerFile := tablesFileName(*outputDir)
	...
	fileHash := hashGenFiles(root, tablesFile, headerFile) // hash gentables source files
	foldHash := prefix()
